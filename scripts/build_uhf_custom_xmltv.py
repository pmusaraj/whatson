...
def source_xml_files() -> list[Path]:
    ...
    # source currently selected for FR/US/UK channels after a source switch.
    ...
    if plan_path.exists():
        ...
        files = [path for path in files if not path.name.startswith(("guide-uhf-US-", "guide-uhf-UK-", "guide-uhf-FR-"))
